...
<div class="space-y-8">
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6">
        <form method="POST" action="{{ route('admin.atividades.update', $evento->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('created_by', $evento->created_by) == $user->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    >
                        <option value="pendente" {{ old('status', $evento->status) === 'pendente' ? 'selected' : '' }}>Pendente</option>
                        <option value="aprovado" {{ old('status', $evento->status) === 'aprovado' ? 'selected' : '' }}>Aprovado</option>
                        <option value="rejeitado" {{ old('status', $evento->status) === 'rejeitado' ? 'selected' : '' }}>Rejeitado</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
